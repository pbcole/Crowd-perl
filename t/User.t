...
my $res = $foo->authenticate("philperl", "testtest");
is(ref($res), "Crowd::User", "Authenticating a valid user returns a Crowd::User object");
SKIP: {
	skip "make it quicker", 1;
my $res2 = $foo->authenticate("philperl", "testtesttest");
is(ref($res2), "Crowd::Error", "Authenticating an invalid user returns a Crowd::Error object");

diag "Valid log in returned: ";
print_results($res);
#$res = $foo->authenticate("philperl", "test");
#print_results($res);
#$res = $foo->authenticate("philprl", "test");
#print_results($res);
...
$res = $foo->lookup_user("philperl", 1);
...
$res = $foo->send_password_reset("philperl");
is(ref($res), "Crowd::Message", "Password reset message sent");
print_results($res);
$res = $foo->send_password_reset("philperlblah");
...
my $user = Crowd::User->new(name => 'philperl', crowd => $foo);
...
#http://crowd:8095/crowd/rest/usermanagement/latest/authentication?username=philperl
